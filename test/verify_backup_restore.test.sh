...
printf '%s\n' 'ORIGINAL NOVNC TEMPLATE' > "$NOVNC_INDEX_TPL"
...
printf '%s\n' 'CHANGED NOVNC TEMPLATE' > "$NOVNC_INDEX_TPL"
...
printf '%s\n' 'new noVNC enhancement' > "${NOVNC_PROXMORPH_DIR}/proxmorph-novnc.js"
...
check 'noVNC template is restored byte-for-byte' 'ORIGINAL NOVNC TEMPLATE' "$(cat "$NOVNC_INDEX_TPL")"
check 'new noVNC enhancement directory is removed' no "$([[ -e "$NOVNC_PROXMORPH_DIR" ]] && echo yes || echo no)"

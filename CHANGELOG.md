...
- **Per-account typography controls**: adds Proxmox-default and modern native font choices plus 13 px, 14 px, and 15 px interface scales. The setting applies across every ProxMorph theme, follows the authenticated PVE account through pmxcfs, preserves Font Awesome icons and monospace consoles, and falls back safely when a saved value is no longer supported.
- **Shared PVE modernization layer**: gives every ProxMorph PVE theme a fluid semantic component system for responsive control density, borderless action toolbars, understated active tabs, elevated menus, focused forms, structured data grids, and a slim theme-colored selection edge while retaining each theme's own palette.
...
### Changed
- Bumped the installer version to 2.20.0.

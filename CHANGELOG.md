...
- **Native noVNC clipboard workflow**: enhances Proxmox's supported `clipboard=vnc` transport with theme-aware toolbar actions, a Shift + right-click console menu that preserves normal guest right-click, and opt-in Ctrl+C/Ctrl+V capture. Clipboard preferences follow the authenticated PVE account while clipboard text remains memory-only and is cleared on disconnect.
...
### Changed
- Bumped the installer version to 2.19.0.

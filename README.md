...
The modal's **Typography** section can retain the native Proxmox font or use a cleaner native UI stack that prefers Roboto Flex, Segoe UI Variable, and the browser operating system's interface font. Text size can remain at 13 px or move to the coordinated 14 px comfortable and 15 px large scales. These rules apply after the active theme, so colors stay theme-specific while typography remains consistent across every ProxMorph theme. Font Awesome icons and monospace console/code surfaces are explicitly left on their purpose-built fonts.
